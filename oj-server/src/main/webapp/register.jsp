...
<%
	...
	if (user.isRegister()) {
		if (user.register()) {
			user.login();
			user.go("Successfully registered.", "/");
		} else {
			user.go("Passwords are not matching, passwords are too long, username has been taken, or username is too long. Please try again.",
					"/register");
		}
	}
%>
...
<head>
<meta charset="utf-8" />
<title>Boleyn Su's Online Judge - Register</title>
